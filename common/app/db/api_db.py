...
@get_pool_cur
async def get_admin_credentials_by_username(cur: Cursor, username: str):
    cur.row_factory = dict_row
    await cur.execute(
        """
...
async def update_pixel(
    ...
    if not permission:
        if (
            user
            and user["last_pixel_update"]
            and (action_time - user["last_pixel_update"]).total_seconds() < cfg.COOLDOWN
        ):
            return "cooldown"
